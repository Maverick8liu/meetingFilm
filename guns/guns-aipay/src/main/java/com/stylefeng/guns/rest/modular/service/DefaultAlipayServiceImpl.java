...
import com.stylefeng.guns.alipay.AliPayServiceAPI;
...
@Service(interfaceClass = AliPayServiceAPI.class,
    mock = "com.stylefeng.guns.alipay.AliPayServiceMock")
public class DefaultAlipayServiceImpl implements AliPayServiceAPI{
    ...
    //测试mock
       /* try {
        ...
        // 获取订单支付状态
        boolean isSucess = trad_query(orderId);
        if (isSucess){
    ...
    // 测试当面付2.0查询订单
    private boolean trad_query(String orderId) {
        boolean flag = false;
...
//        goodsDetailList.add(goods2);
        ...
        switch (result.getTradeStatus()){
            case SUCCESS:
                log.info("alipay pre order success:");
                AlipayTradePrecreateResponse response = result.getResponse();

                //need modify run machine filePath
                filePath = String .format("C:\\Users\\gxw16\\Desktop\\QRCode\\qr-%s.png",
                ...
                boolean isSuccess = ftpUtil.uploadFile(fileName,qrCodeImage);
